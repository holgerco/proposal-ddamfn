...
def run_training():
    args = Args_Info(fer_path=None, batch_size=128, lr=0.01, workers=8, epochs=25, num_head=2)
    ...
    optimizer = torch.optim.Adadelta(params, lr=args.lr, weight_decay=1e-4)
        ...
        with torch.no_grad():
            ...
            if acc > 0.8 and acc == best_acc:
                torch.save({'iter': epoch,
                           ...
                           os.path.join('checkpoints', "ferPlus_epoch" + str(epoch) + "_acc" + str(acc) + "_bacc" + str(
                               balanced_acc) + ".pth"))
                ...
                plt.savefig(os.path.join('checkpoints',
                                         "ferPlus_epoch" + str(epoch) + "_acc" + str(acc) + "_bacc" + str(
                                             balanced_acc) + ".png"))
                plt.close()
